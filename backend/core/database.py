...
class DatabaseManager:
    ...
    async def init_db(self):
        ...
        # Allow explicit environment variable fallback when Settings doesn't expose the attribute
        database_url = settings.database_url if getattr(settings, "database_url", None) else os.environ.get(
            "DATABASE_URL"
        )
        ...
        if not database_url:
            ...
            logger.warning(
                "DATABASE_URL not set; using local SQLite database at %s",
                default_db_path,
            )
