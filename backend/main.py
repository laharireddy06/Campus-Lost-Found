...
async def lifespan(app: FastAPI):
    ...
    # Clean up specific requested items from local database files
    try:
        import sqlite3
        for db_name in ["data.db", "dev.db"]:
            if os.path.exists(db_name):
                conn = sqlite3.connect(db_name)
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM items WHERE title LIKE '%gold bangle%' OR title LIKE '%laptop%' OR title LIKE '%blue bag%' OR title LIKE '%aadhar card%'"
                )
                conn.commit()
                logger.info(f"DB CLEANUP: Deleted requested mock items from {db_name}. Rowcount: {cursor.rowcount}")
                conn.close()
    except Exception as e:
        logger.error(f"DB CLEANUP ERROR: Failed to remove mock items: {e}")
    # MODULE_STARTUP_END
